...
function ListingDetailsPopup({ listing, onClose }) {
  ...
  const fetchUserEmail = async () => {
    try {
      const response = await fetch(`${BACKEND_ADDRESS}/api/get_user_email/`, {
        method: 'GET',
        headers: {
    ...
    <div className="popup-card">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        {isLoggedIn && ( // Render the button only if user is logged in
          <button style={{ marginTop: '10px' }} className="view-email-button" onClick={fetchUserEmail}>
            View Email
          </button>
        )}
      </div>
      {userEmail && (
        <p className="popup-card-text">User Email: {userEmail}</p>
      )}
    </div>
  );
}
